<?php 
//    echo basename($_SERVER['PHP_SELF']);
    include 'config.php';
 
   $page = basename($_SERVER['PHP_SELF']);
   switch ($page) {
    case "single.php":
        if(isset($_GET['id'])){
            $sql_title ="SELECT * FROM post WHERE post_id = {$_GET['id']}";
            $resutl_title = mysqli_query($conn,$sql_title) or die("Query Faield");
            $row_title = mysqli_fetch_assoc($resutl_title);
            $page_title = $row_title['title'];
        }else{
            $page_title = "No Page Found";
        }
        break;

    case "category.php":
        if(isset($_GET['cid'])){
            $sql_title ="SELECT * FROM category WHERE category_id = {$_GET['cid']}";
            $resutl_title = mysqli_query($conn,$sql_title) or die("Query Faield");
            $row_title = mysqli_fetch_assoc($resutl_title);
            $page_title = $row_title['category_name'];
        }else{
            $page_title = "No Page Found";
        }
        break;

    case "auther.php":
        if(isset($_GET['aid'])){
            $sql_title ="SELECT * FROM users WHERE user_id = {$_GET['aid']}";
            $resutl_title = mysqli_query($conn,$sql_title) or die("Query Faield");
            $row_title = mysqli_fetch_assoc($resutl_title);
            $page_title = $row_title['username'];
        }else{
            $page_title = "No Page Found";
        }
        break;

    case "search.php":
        if(isset($_GET['search'])){
            $page_title = $_GET['search'];
        }else{
            $page_title = "No Page Found";
        }
        break;   
        
        default :
        $page_title = "News Site";
        
   }
//    echo $page_title;

?>

<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">

    <title> <?php echo $page_title; ?> </title>

    <link rel="stylesheet" href="./css/style.css">
</head>
<body>
    <section id="home_header"> 
    <?php
            include "config.php";

            $sql = "SELECT * FROM settings";

            $result = mysqli_query($conn,$sql) or die("Connection Failed");

            if(mysqli_num_rows($result) > 0 ){
                while($row = mysqli_fetch_array($result)){
                    if($row['logo']==""){
                        echo "<a href='index.php'><h1>" . $row['websitename'] . "</h1></a>";
                    die();
                    }else{
                       echo '<a href="index.php">
                            <img src="admin/image/'.$row['logo'].'">
                       </a>';
                    }
        ?>
            
            
            
        <?php 
                }
            }
        ?>
    </section>
        

    <section id="home_menu">
        <?php 
            include 'config.php';

            if(isset($_GET['cid'])){
                $cat_id = $_GET['cid'];
            }


            $sql = "SELECT * FROM category WHERE post > 0";

            $result = mysqli_query($conn,$sql)or die("Query Failed : Caregory") ;
            
            if(mysqli_num_rows($result) > 0) {
                
                $active = "";
        
        ?>
        <div>
            <ul>
                <li><a href="index.php">Home</a></li>
            <?php while($row = mysqli_fetch_array($result)) { 

                if(isset($_GET['cid'])){
                    if($row['category_id']==$cat_id){
                        $active = "active";
    
                    }else{
                        $active = "";
                    }
                }

                echo "<li  ><a class='{$active}' href='category.php?cid={$row['category_id']}'> {$row['category_name']} </a></li>";
                
                }
            ?>
            
            </ul>
        </div>
        <?php 
                }
            ?>
    </section>




</body>
</html>